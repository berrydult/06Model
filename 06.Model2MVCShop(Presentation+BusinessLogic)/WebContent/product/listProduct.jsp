	...
	<tr>
		...
		<td align="right">
			...
			<input type="text" name="searchKeyword"
			 value="${! empty search.searchKeyword ? search.searchKeyword : ""}"  class="ct_input_g" style="width:200px; height:19px" />
		</td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	<tr class="ct_list_pop">
		...
		<td></td>
		<c:if test= "${param.menu == 'manage'}">
		<td align="left"><a href="/updateProductView.do?prodNo=${product.prodNo}&menu='manage'">
																	${product.prodName}</a></td>
		</c:if>
		<c:if test= "${param.menu == 'search'}">
		<td align="left"><a href="/getProduct.do?prodNo=${product.prodNo}&menu='search'">
																	${product.prodName}</a></td>
		</c:if>
		...
		<c:if test = "${user.role == 'admin'}">
			...
			<c:if test = "${fn:trim(product.proTranCode) =='1' && param.menu == 'manage'}">
	  			판매완료<a href = "/updateTranCodeByProd.do?prodNo=${product.prodNo}&tranCode=2">배송하기</a>
	  		</c:if>
